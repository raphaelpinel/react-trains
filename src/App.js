...
class App extends Component {
  ...
  fetchStations() {
    ...
    .then(
      data => { // the React Select component needs data in the format { value: xxx, label: yyy }
        const stations = data.map(station => ({ value: station.stationShortCode, label: station.stationName.includes(' asema') ? station.stationName.slice(0, -6) :  station.stationName})); // removing ' asema' from station names
        
        const passengerStations = stations.filter(station => station.passengerTraffic === true) //filtered for only passenger traffic
        .map(station => ({ value: station.stationShortCode, label: station.stationName.includes(' asema') ? station.stationName.slice(0, -6) :  station.stationName}));
  ...
  filterData(selectedStation) { 
    ...
    const filteredData = todaysTrains.map((train => {
      ...
      const origin = stations.find(station => station.value === originShortCode).label; // retrieve the full name of station by short code
